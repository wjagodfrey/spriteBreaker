// Generated by CoffeeScript 1.7.1
(function() {
  var app, backgroundTile, drawBackgroundTiles, fakeImageData, fileSelect, frame, imageCanvas, imageMouseCoords, imageSelection, imageSelector, imagedata, img, imgResizeFactor, jsonoutput, reset, tileImg, zoomSpeed, zoomedCanvas, zoomedMouseCoords;

  app = angular.module('app', []);

  fakeImageData = 'data:image/png;base64,';

  frame = 0;

  zoomSpeed = 0.03;

  imgResizeFactor = 0;

  imageSelector = {};

  imageSelection = {};

  imageMouseCoords = {};

  zoomedMouseCoords = {};

  img = new Image();

  imageCanvas = $('canvas#navigator');

  imageCanvas[0].width = imageCanvas.parent().width();

  imageCanvas[0].height = 200;

  zoomedCanvas = $('canvas#selector');

  zoomedCanvas[0].width = zoomedCanvas.parent().width();

  zoomedCanvas[0].height = 250;

  imagedata = $('#imagedata');

  fileSelect = $('#filepath');

  jsonoutput = $('#output');

  backgroundTile = void 0;

  tileImg = new Image();

  tileImg.onload = function() {
    return backgroundTile = cq().context.createPattern(tileImg, 'repeat');
  };

  tileImg.src = './img/tile.png';

  drawBackgroundTiles = function(ctx) {
    if (backgroundTile != null) {
      return ctx.rect(0, 0, ctx.canvas.width, ctx.canvas.height).fillStyle(backgroundTile).fill();
    }
  };

  reset = function() {
    imageSelector = {
      color: '#de683c',
      zoom: 1
    };
    imageSelection = {
      color: '#29a4d3'
    };
    imageMouseCoords = {
      x: imageCanvas.innerWidth / 2,
      y: imageCanvas.innerHeight / 2
    };
    return zoomedMouseCoords = {
      x: 0,
      y: 0
    };
  };

  imageCanvas.on('mousewheel', function(e) {
    e.preventDefault();
    if (e.deltaY) {
      imageSelector.zoom += zoomSpeed * (e.deltaY < 1 ? 1 : -1);
      if (imageSelector.zoom < 0.1) {
        imageSelector.zoom = 0.1;
      }
      if (imageSelector.zoom > 1) {
        return imageSelector.zoom = 1;
      }
    }
  });

  fileSelect.on('change', function(e) {
    var reader, selectedFile;
    selectedFile = e.target.files[0];
    reader = new FileReader();
    reader.onload = function(e) {
      return scope.$apply(function() {
        return scope.imagedata = e.target.result;
      });
    };
    return reader.readAsDataURL(selectedFile);
  });

  app.controller('appCtrl', [
    '$scope', function(scope) {
      var imageCq, zoomedCq;
      scope.sprites = [];
      imageCq = cq(imageCanvas[0]).framework({
        onmouseup: function() {
          imageSelection.zoom = imageSelector.zoom;
          imageSelection.x = imageSelector.x;
          return imageSelection.y = imageSelector.y;
        },
        onmousemove: function(x, y) {
          return imageMouseCoords = {
            x: x,
            y: y
          };
        },
        onrender: function(delta, time) {
          this.context.mozImageSmoothingEnabled = this.context.webkitImageSmoothingEnabled = this.context.msImageSmoothingEnabled = this.context.imageSmoothingEnabled = false;
          this.clear();
          drawBackgroundTiles(this);
          if (img.src) {
            imgResizeFactor = Math.min(this.canvas.height / img.height, this.canvas.width / img.width);
            imageSelector.width = zoomedCq.canvas.width * imageSelector.zoom;
            imageSelector.height = zoomedCq.canvas.height * imageSelector.zoom;
            imageSelector.x = imageMouseCoords.x - imageSelector.width / 2;
            imageSelector.y = imageMouseCoords.y - imageSelector.height / 2;
            this.save().translate((this.canvas.width - img.width * imgResizeFactor) / 2, (this.canvas.height - img.height * imgResizeFactor) / 2).drawImage(img, 0, 0, img.width * imgResizeFactor, img.height * imgResizeFactor).restore().save().translate(imageSelector.x, imageSelector.y).save().globalAlpha(0.3).fillStyle(imageSelector.color).fillRect(0, 0, imageSelector.width, imageSelector.height).restore().strokeStyle(imageSelector.color).strokeRect(0, 0, imageSelector.width, imageSelector.height).restore();
            if (imageSelection.zoom != null) {
              return this.save().translate(imageSelection.x, imageSelection.y).save().globalAlpha(0.3).fillStyle(imageSelection.color).fillRect(0, 0, zoomedCanvas.width() * imageSelection.zoom, zoomedCanvas.height() * imageSelection.zoom).restore().strokeStyle(imageSelection.color).strokeRect(0, 0, zoomedCanvas.width() * imageSelection.zoom, zoomedCanvas.height() * imageSelection.zoom).restore();
            }
          }
        }
      });
      zoomedCq = cq(zoomedCanvas[0]).framework({
        onmousemove: function(x, y) {
          return zoomedMouseCoords = {
            x: x,
            y: y
          };
        },
        onrender: function(delta, time) {
          this.context.mozImageSmoothingEnabled = this.context.webkitImageSmoothingEnabled = this.context.msImageSmoothingEnabled = this.context.imageSmoothingEnabled = false;
          this.clear();
          drawBackgroundTiles(this);
          if (imageSelection.zoom != null) {
            this.drawImage(img, (imageSelection.x - (imageCanvas.width() - img.width * imgResizeFactor) / 2) / imgResizeFactor, (imageSelection.y - (imageCanvas.height() - img.height * imgResizeFactor) / 2) / imgResizeFactor, (zoomedCanvas.width() * imageSelection.zoom) / imgResizeFactor, (zoomedCanvas.height() * imageSelection.zoom) / imgResizeFactor, 0, 0, this.canvas.width, this.canvas.height);
            if (frame < 10 && img.width) {
              return frame++;
            }
          }
        }
      });
      scope.imagedata = fakeImageData;
      return scope.output = 'JSON\nhere';
    }
  ]);

}).call(this);
