...
(function() {
  var app, backgroundTile, drawBackgroundTiles, fakeImageData, fileSelect, frame, imagedata, img, imgResizeFactor, jsonoutput, navigatorCanvas, navigatorCursor, navigatorMouseCoords, navigatorSelection, reset, selectorCanvas, selectorMouseCoords, tileImg, zoomSpeed;
      ...
      navigatorCq = cq(navigatorCanvas[0]).framework({
        ...
        onrender: function(delta, time) {
          ...
          if (img.src) {
            ...
            if (navigatorSelection.zoom != null) {
              this.save().translate(navigatorSelection.x, navigatorSelection.y).save().globalAlpha(0.3).fillStyle(navigatorSelection.color).fillRect(0, 0, selectorCanvas.width() * navigatorSelection.zoom, selectorCanvas.height() * navigatorSelection.zoom).restore().strokeStyle(navigatorSelection.color).strokeRect(0, 0, selectorCanvas.width() * navigatorSelection.zoom, selectorCanvas.height() * navigatorSelection.zoom).restore();
            }
            if (frame++ < 10) {
              return console.log(selectorCq.canvas.width, navigatorCursor.zoom);
            }
          }
        }
      });
